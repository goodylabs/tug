...
func TestGetSingleIpFromShellFile(t *testing.T) {
	...
	t.Run("IP_ADDRESS variable", func(t *testing.T) {
		scriptPath := filepath.Join(devopsDirPath, "localhost", dockercommon.DEPLOY_FILE)

		ip := services.GetSingleIpFromShellFile(scriptPath, dockercommon.IP_ADDRESS_VAR)

		assert.Equal(t, "unix:///var/run/docker.sock", ip)
	})

	t.Run("TARGET_IP variable", func(t *testing.T) {
		scriptPath := filepath.Join(devopsDirPath, "production", dockercommon.DEPLOY_FILE)

		ip := services.GetSingleIpFromShellFile(scriptPath, dockercommon.TARGET_IP_VAR)
...
func TestGetMultipleIpsFromShellFile(t *testing.T) {
	...
	t.Run("IP_ADDRESS variable", func(t *testing.T) {
		scriptPath := filepath.Join(devopsDirPath, "production_v2", dockercommon.DEPLOY_FILE)

		ip := services.GetMultipleIpsFromShellScript(scriptPath, dockercommon.IP_ADDRESSES_VAR)
